...
select listagg(case
                when l.chart_row = 44                                     then '_|   |'
                when l.chart_row = 45                                     then ' ' || rpad(d.lbl, 5, ' ') || ''
                when round(43 - (d.val/d.val_max * 40)) = l.chart_row + 1 then ' ' || lpad(d.val, 5, ' ') || ''
                when round(43 - (d.val/d.val_max * 40)) = l.chart_row     then ' ,___,'
